    ...
    <div className="px-4 pt-6 pb-4 max-w-lg mx-auto">
      ...
      {plans.length === 0 ? (
        <div className="card p-8 text-center">
          <p className="text-neutral-400 mb-4">No plans yet. Create one to get started.</p>
          <Link href="/plans/new">
          ...
          {plans.map(plan => (
            <div key={plan.id} className="card p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-sm text-neutral-500">
                    {plan.exercises?.length || 0} exercises · Created {new Date(plan.created_at).toLocaleDateString()}
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setDeleteId(plan.id); }} className="btn-ghost text-red-400 hover:text-red-300 text-sm min-h-11">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
