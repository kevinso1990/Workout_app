...
function clientKey(req: Request): string {
  // `req.ip` honors Express's `trust proxy` setting (configured in server/index.ts
  // to trust exactly 1 upstream hop). Do NOT read `x-forwarded-for` directly —
  // a malicious client can spoof it to bypass per-IP limits.
