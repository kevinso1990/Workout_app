...
export const logSet = asyncHandler(async (req: Request, res: Response) => {
  res.json(setService.logSet(req.body as LogSetBody, req.user!.sub));
});

export const updateRir = asyncHandler(async (req: Request, res: Response) => {
  const { rir } = req.body as { rir: number };
  setService.updateSetRir(parseId(req.params.id), rir, req.user!.sub);
  res.json({ ok: true });
});

export const remove = asyncHandler(async (req: Request, res: Response) => {
  setService.deleteSet(parseId(req.params.id), req.user!.sub);
  res.json({ ok: true });
});
