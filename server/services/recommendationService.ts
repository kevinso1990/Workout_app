...
function assertOwnsPlan(planId: number, userId: number): void {
  const row = db
    .prepare("SELECT 1 FROM plans WHERE id = ? AND user_id = ?")
    .get(planId, userId) as { 1: number } | undefined;
  if (!row) throw new AppError(404, "Plan not found");
}

export function getRecommendations(planId: number, userId: number): Recommendation[] {
  assertOwnsPlan(planId, userId);
  ...
  const lastSession = db
    .prepare(
      `SELECT id, rpe FROM sessions
       WHERE plan_id = ? AND user_id = ? AND finished_at IS NOT NULL
       ORDER BY finished_at DESC LIMIT 1`,
    )
    .get(planId, userId) as { id: number; rpe: number | null } | undefined;
