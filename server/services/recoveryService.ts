 ...
 * on the sets logged and stores them in the `muscle_fatigue` table, scoped to
 * the session's owner.
 */
export function calculateAndStoreFatigue(sessionId: number, userId: number): void {
  ...
  const insert = db.prepare(
    "INSERT INTO muscle_fatigue (muscle_group, fatigue_score, last_trained_at, session_id, user_id) VALUES (?, ?, ?, ?, ?)",
  );

  db.transaction(() => {
    for (const [muscle, score] of Object.entries(fatigueMap)) {
      insert.run(muscle, score, session.finished_at, sessionId, userId);
    }
  })();
}
...
export function getRecovery(userId: number): RecoveryStatus[] {
  ...
  const rows = db
    .prepare(
      `SELECT muscle_group, fatigue_score, last_trained_at
       FROM muscle_fatigue
       WHERE user_id = ?
         AND last_trained_at > datetime('now', '-20 days')
       ORDER BY last_trained_at DESC`,
    )
    .all(userId) as MuscleFatigueRow[];
