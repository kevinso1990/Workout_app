...
export function getSplitAge(
  userId: number,
  deviceId: string,
  thresholdWeeks = DEFAULT_THRESHOLD_WEEKS,
): SplitAge | null {
  // Find the plan THIS user has used most in the last 8 weeks. Without the
  // user_id filter, this would surface another user's most-used plan.
  const row = db
    .prepare(
