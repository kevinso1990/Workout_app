...
beforeAll(async () => {
  ...
  const userRow = db
    ...
    .get("stats@test.com") as { id: number };
  statsUserId = userRow.id;

  // Get an exercise ID from the seeded data (public catalog — no JWT required)
  const exRes = await request(app).get("/api/exercises/catalog");
...
async function logSession(startedAt: string, finishedAt: string, sets: Array<{ weight: number; reps: number }>) {
  ...
  const sessionRow = db
    ...
    .run(planId, statsUserId, startedAt, finishedAt);
